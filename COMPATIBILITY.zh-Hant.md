...
1.10.45 與 TeslaMate 4.2.0、上一個穩定版 4.1.1 及 TeslaMateAPI 1.25.0
...
1.10.45 加入選用、預設關閉的朋友同行。從 1.10.44 用 HostBox 或命令列升級後，
停車、導航、配對與通知維持原樣，不需額外設定。之後若要啟用朋友同行，需設定
`FRIEND_TOGETHER_ENABLED=true`、獨立 HTTPS 訪客網域（只公開 `/friend/v1/*`），
並在已驗證 API 站點加入 `/api/v1/friend-together/*`。不需要遷移資料庫、配對、
Tesla 權杖、通知偏好或既有歷史，並繼續相容現有 My T 用戶端。
