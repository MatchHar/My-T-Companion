...
安全配對後，每支 iPhone 可以個別開啟「上鎖且車內無人」通知。該偏好適用於該
iPhone 所配對 TeslaMate 伺服器上的全部車輛；在 My T 切換目前選擇的車輛不會
篩掉其他車輛的伺服器端通知。Companion 只會傳送簽署事件；可見訊息及提示音由
My T 在手機上處理。匯入音訊、檔名及靜音／預設選擇均不會傳送到 Companion 或
推播中繼。

每支 iPhone 也可以個別開啟低電量提醒；該偏好同樣涵蓋其所配對 TeslaMate
伺服器上的全部車輛。任一車輛停妥、未充電且電量嚴格低於 20% 時提醒一次，低於
