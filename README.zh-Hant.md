...
安全配對後，每支 iPhone 可以個別開啟「上鎖且車內無人」通知。Companion 只會
傳送簽署事件；可見訊息及提示音由 My T 在手機上處理。匯入音訊、檔名及靜音／
預設選擇均不會傳送到 Companion 或推播中繼。

每支 iPhone 也可以個別開啟低電量提醒：車輛停妥、未充電且電量嚴格低於 20% 時
提醒一次，低於 10% 時可再發一次較強提醒，電量達到 25% 後才重新布防。第一個完整
