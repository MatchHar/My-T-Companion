...
安全配对后，每台 iPhone 可以独立开启“锁车且车内无人”通知。该偏好适用于该
iPhone 所配对 TeslaMate 服务器上的全部车辆；在 My T 切换当前选择的车辆不会
筛掉其他车辆的服务器端通知。Companion 只发送签名事件；可见消息和提示音由
My T 在手机上处理。导入音频、文件名及静音／默认选择都不会发送到 Companion
或推送中继。

每台 iPhone 也可以独立开启低电量提醒；该偏好同样覆盖其所配对 TeslaMate
服务器上的全部车辆。任一车辆停车、未充电且电量严格低于 20% 时提醒一次，低于
