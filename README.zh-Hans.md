...
安全配对后，每台 iPhone 可以独立开启“已上锁且未检测到人员”提醒。人员检测仅供
参考，不能证明车内无人，请自行确认所有座位；当前证据不足时不会发送提醒。服务器上的选择是
全部车辆的预设；兼容的 My T 版本可按车辆名称，为每一类通知分别建立覆盖，新加入
车辆会自动沿用预设。在 My T 切换当前选择的车辆不会改变服务器端通知范围。
Companion 只发送签名事件；可见消息和提示音由
My T 在手机上处理。导入音频、文件名及静音／默认选择都不会发送到 Companion
或推送中继。

每台 iPhone 也可以独立开启低电量提醒；该偏好默认覆盖其所配对 TeslaMate
服务器上的全部车辆，也可为单一车辆覆盖。任一车辆停车、未充电且电量严格低于 20% 时提醒一次，低于
10% 时可再发一次更强提醒，电量达到 25% 后才重新布防。首次完整
MQTT 保留快照只建立基线，不会在安装或重启时误报。通知可选择“知道了”或“4 小时
后提醒”；操作状态按手机保存在用户自己的 VPS，充电会取消待提醒。Companion 只读
TeslaMate 已上报的数据，不轮询 Tesla，也不会唤醒车辆。

**本组件专为
[My T iPhone App 开发，可在 App Store 下载](https://apps.apple.com/app/id6780299502)。**
如果您是从 TeslaMate 项目找到这里，请先通过此链接确认并下载配套的 My T App。

**需要部署服务器栈？[从 App Store 下载 HostBox](https://apps.apple.com/app/id6798103086)。** [产品说明与上线视频](https://my-tesla.app/hostbox/)介绍了如何直接在 iPhone 引导部署 VPS。
...
正数 TeslaMate car_id 和完整的分类开关；未列出的车辆沿用顶层全部车辆预设。
省略字段会保留已有覆盖，避免旧版 App 清除自己无法显示的设置；明确发送空数组才
清除全部覆盖。
...
完成推送配对后，Companion 还会提交用于运营统计的匿名车辆清单。它在自己的
数据卷中生成并保存随机命名空间，再为该已配对服务器上的每辆 TeslaMate 车辆
通过 HMAC 派生一个稳定别名。中继只保留别名及首次／最后出现时间，不会把别名
与安装 ID 一起保存，也不会接收原始车辆 ID、车辆名称、VIN、服务器地址、Apple
ID、手机身份、位置、路线、遥测、软件版本或通知内容。同一台车在一个 Companion
配对多台 iPhone 时仍只计算一次。
...
| 路径 | 对象 | 说明 |
| --- | --- | --- |
| **[HostBox](https://apps.apple.com/app/id6798103086)**（多数手机用户推荐） | App Store 上的 iOS 部署 App | 安装 TeslaMate + API + 本组件，尽量配置**统一入口**（临时 IP 的 edge，或 Tunnel 路径）、MQTT `host.docker.internal` 与健康检查。完成后在 My T 只填**一个** base_url + Token；详见[产品说明](https://my-tesla.app/hostbox/)。 |
...
[My T](https://apps.apple.com/app/id6780299502) 是用于查看用户自建
...
安装程序可以自动处理能够识别的系统 Caddy，也可以处理与 Companion 共用已验证
Docker 网络的 TeslaMate 容器版 Caddy。系统网关转到 `127.0.0.1:8083`；容器版
Caddy 则在不放宽主机 8083 本地绑定的前提下直连 `companion:8080`。Nginx、
Traefik、无法识别的容器代理或自定义内网网关仍需要管理员手动加入路由；只安装
容器、不配置统一入口时，My T 不会显示增强功能。
...
| Companion 版本 | 新增能力 |
| --- | --- |
| 1.10.47 | 朋友同行查车改用已可用的 TeslaMate Postgres 连接串 |
| 1.10.46 | 安装程序把 `internal/` 拷入 Docker 构建目录，修复 HostBox 升级失败 |
| 1.10.45 | 可选默认关闭的朋友同行；常规升级不改变停车、导航与配对 |
| 1.10.44 | 拒绝到达时旧目的地回跳，并以限时确认识别邻近下一站，重定向时不沿用上一段路线数据 |
| 1.10.43 | 停车事件按顺序快速接收、合并持久化，并在服务正常停止时完整写入 |
| 1.10.39 | 充电、导航与锁车安心推送加入数据来源隔离，并避免实时活动会话冲突 |
...
官方压缩包及校验文件也带有 GitHub 构建来源证明。安装前可独立验证已下载文件：
...
Nginx、Traefik 或无法识别的容器代理时，必须加入并验证仓库提供的路由。安装器还会：
...
- 自动处理可识别的系统 Caddy，或共用已验证 TeslaMate 网络的容器化 Caddy。
- 对无法安全识别的 Nginx、Traefik、其他容器代理或自定义布局停止自动修改，
...
本仓库源代码按 [MIT License](LICENSE) 公开，可依许可自行托管、修改及再分发。
My T iOS 源代码、开发者运营的中继实现与 APNs 提供方凭据、HostBox App 与部署
源代码，以及 HostBox 目录签名私钥，均为不包含在本仓库内的独立私有组件。

MIT 许可不会使派生版本成为 My T 或 HostBox 官方版本。再分发的派生版本应使用
清晰不同的产品品牌，不得以暗示官方认可或官方构建的方式使用 My T／HostBox 的
名称、图标或整体呈现。
