...
1.10.45 与 TeslaMate 4.2.0、上一稳定版 4.1.1 以及 TeslaMateAPI 1.25.0
...
1.10.45 加入可选、默认关闭的朋友同行。从 1.10.44 用 HostBox 或命令行升级后，
停车、导航、配对与通知保持原样，无需额外配置。之后若要启用朋友同行，需设置
`FRIEND_TOGETHER_ENABLED=true`、独立 HTTPS 访客域名（只公开 `/friend/v1/*`），
并在已认证 API 站点加入 `/api/v1/friend-together/*`。不需要迁移数据库、配对、
Tesla 令牌、通知偏好或已有历史，并继续兼容现有 My T 客户端。
